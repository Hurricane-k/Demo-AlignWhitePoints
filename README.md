...
5. ⭐🌟`mainFunc.m` generates `./data/Results.mat` but not the ultimate results. The ultimate result need to be processed to get indices `SPDLab(:,IllumFreq(:))`: ***the ultimate results** (**several representative SPDs**)* at the end of `mainFunc.m` (*using the following code cell to get several representative SPDs*):
```
clear all
